 ...
 *
 * @param {ReturnType<typeof checkIntegrateGate>} gateCheck
 */
function gateBlockedOnMissingGate(gateCheck) {
	return (
		gateCheck.gate == null &&
 ...
 * @param {ReturnType<typeof loadSpineConfig>["config"]|null} ctx.config
 * @returns {{ ok: true, opened: boolean, gate: object, evidenceRefs: string[] } | { ok: false, error: string }}
 */
function openGateFromSalvageEvidence(projectRoot, batchId, ctx) {
...
export async function integrateSalvageableLane(projectRoot, batchId, laneNumber, options = {}) {
	...
	let gateCheck = checkIntegrateGate({
		projectRoot,
		batchId: resolvedBatchId,
		config,
		forceIntegrate,
	...
	// #274 — a batch that failed before merge never opened the integrate gate,
	// so the old path dead-ended on "no gate record". When the lane is salvageable,
	// open a fresh gate from salvage evidence instead of failing.
	let gateOpenedBySalvage = false;
	if (!gateCheck.ok && gateBlockedOnMissingGate(gateCheck)) {
		const openResult = openGateFromSalvageEvidence(projectRoot, resolvedBatchId, {
			lane,
			baseBranch,
			taskBranch,
			commitsAhead,
		...
		if (!openResult.ok) {
			...
			return {
				...
				error: openResult.error,
				headline: "Salvage integrate blocked — could not open integrate gate from salvage evidence",
				suggestedCommand: "spine status --diagnose",
		...
		gateOpenedBySalvage = true;
		gateCheck = checkIntegrateGate({ projectRoot, batchId: resolvedBatchId, config, forceIntegrate });
	}
	...
	if (!gateCheck.ok) {
		...
		return {
			...
			headline: gateOpenedBySalvage
				? "Salvage opened a fresh integrate gate — approve it, then re-run salvage integrate"
				: gateCheck.headline ?? "Salvage integrate blocked by gate",
			suggestedCommand: gateCheck.suggestedCommand ?? "spine gate approve",
			alternatives: gateOpenedBySalvage
				? [
						"/spine-gate approve",
						`spine batch salvage --batch ${resolvedBatchId} --lane ${laneNum} --integrate --yes`,
					]
				: gateCheck.alternatives ?? ["/spine-gate approve"],
			batchId: resolvedBatchId,
			laneNumber: laneNum,
...
export function formatSalvageIntegrateOutput(result, options = {}) {
	...
	if (result.gateOpenedBySalvage) {
		lines.push(
			"",
			"  Gate: opened by salvage from inspection evidence (evidence/salvage-inspect.json)",
		);
	}
	if (result.batchId) {
