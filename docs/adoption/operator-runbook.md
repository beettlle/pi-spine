...
- `spine status` (and `--diagnose`) lists each row under the parent as `Matrix rows (SP-X):` with `rowId: status` (#230, SP-752). JSON status includes `signals.tasks[].matrixRows` when present. Journal events remain `matrix.sub_lane.started/completed/failed`.
- The parent task succeeds only if **all non-canceled rows** succeed. If any row fails, the parent task fails with `matrix_sub_lane_failed:<rowIds>` and the failing row IDs are surfaced in the diagnosis.
- **Per-row retry / cancel** (#230, SP-752): `spine batch retry 'SP-X[rowId]'` resets one `failed` row without re-running succeeded siblings; `spine batch skip 'SP-X[rowId]'` cancels one row (`canceled`). Plain `spine batch retry SP-X` / `skip SP-X` still operate on the whole matrix. Pause the batch first when phase is `running`.
...
- **Deferred follow-ups:** `maxFailedIndexes` partial-failure tolerance (#231) remains deferred. Per-row status / retry / cancel (#230) shipped in SP-752 (see Concurrency and failure behavior above). Matrix environment variables and the `matrixMaxParallel` throttle (#229) shipped in SP-751 (see the row environment and throttle sections above). Full PROMPT-body substitution for LLM rows (#232) shipped in SP-742 (see the substitution rules above). The default parent success policy is that all non-canceled rows succeed.
