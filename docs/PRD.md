...
- Decrement `failedTasks`; recompute `blockedTaskIds`
- Set batch phase → `stopped` (resumable with `--force`)
- Journal: `task.retry_requested` with `{ taskId, previousClassification }`

Resume must refuse if any segment for the task remains terminal without matching task state (fail loud: `RetrySegmentDrift`).

### 18.6 Abort and state durability

| Operation | Behavior |
|-----------|----------|
| Graceful abort | Stop scheduling; wait for wrap-up; **archive** batch-state + journal tail |
| Hard abort | Kill sessions; same archive requirement |
| Active state | Remove `.spine/batch-state.json` only **after** archive fsync |
| Reconstruction | `spine state rebuild --batch {id}` from journal + archive (v1); must preserve segment topology |

**Never** delete the only copy of segment records without archive.

### 18.7 Safe retry while batch paused

Operators must be able to retry one failed task without full abort when batch phase is `paused` or `stopped`:

- `/spine-retry-task {id}` allowed in `paused`, `stopped`, `failed`
- Blocked in `executing`, `merging`, `launching` unless `--takeover` first (supervisor pause + drain)

### 18.8 Taskplane compatibility gaps (dogfood findings)

Document and test against Taskplane behavior that pi-spine must **not** replicate:

| Gap | Taskplane behavior | pi-spine requirement |
|-----|-------------------|----------------------|
| GAP-RETRY-01 | Retry task record only | §18.5 atomic retry |
| GAP-ABORT-01 | Deletes `.pi/batch-state.json` | §18.6 archive-first |
...
| ID | Requirement |
|----|-------------|
| SEC-01 | `worktreeSetupHook` disabled by default; explicit opt-in in config |
| SEC-02 | Hook commands logged to journal (command name only, not env) |
| SEC-03 | Evidence bundles world-readable in `.spine/` — document in init |
| SEC-04 | No network calls from spine except dashboard localhost bind |
| SEC-05 | Dependency audit in CI (`npm audit` moderate+ fails publish gate) |
...
| Module | Cases |
|--------|-------|
| planner | cycles, waves, file-scope lanes, maxParallel queue |
| compat/taskplane | heading parse, deps merge, STATUS mirror |
| journal | append, replay order, payload size limit |
| gates | FSM transitions, fail-closed integrate |
| resume | idempotent skip, stale lane repair, segment frontier rebuild |
| retry | atomic task+segment reset, pendingSegments count |
| stall | progress signals suppress kill; false positive fixture |
| abort | archive exists after abort; rebuild preserves segments |

### 20.2 Integration tests (fixture repo)

Fixture: 3 tasks (A, B, C); A independent; B depends A; C independent.

- Plan: wave0 {A,C}, wave1 {B}
- Run 2 lanes; verify worktrees
- Simulate lane-2 stale; resume
- Complete; gate; dry-run integrate

### 20.3 Manual validation (Taskplane migration)

Use an existing `taskplane-tasks/` tree with `--tasks-root taskplane-tasks`:

1. `/spine-plan` on a dependency-chain subset (e.g. three linked tasks)
2. Compare wave structure to Taskplane plan
3. Document gaps in `docs/compatibility/taskplane-gap-list.md`
...
| Doctor smoke | `spine doctor` in fixture repo |
| pi load smoke | `pi` session lists `/spine` |

---

## 21. Repository layout and conventions

### 21.1 pi package manifest
...
### 21.3 Git conventions (consumer)

- Worker commits: `{taskId} step {n}: {title}`
- Spine tooling commits (if any): `spine: {message}`
- Orch branch never force-pushed by spine

---

## 22. Migration from Taskplane

### 22.1 Steps

1. `pi install npm:pi-spine` (optionally `pi remove npm:taskplane` after validation).
2. `spine init --tasks-root taskplane-tasks --preset taskplane-compat`.
3. `spine migrate-from-taskplane --source .pi/taskplane-config.json`.
4. Copy agent overrides: `.pi/agents/task-worker.md` → `.spine/agents/worker.md` (manual or script).
5. `/spine-plan all` — compare to last Taskplane plan.
6. **Never** run Taskplane and pi-spine batches concurrently on same repo.

### 22.2 Config field mapping

| Taskplane (`taskplane-config.json`) | pi-spine (`spine-config.json`) |
|-------------------------------------|--------------------------------|
| `taskRunner.paths.tasks` | `paths.tasksRoot` |
| `taskRunner.testing.*` | `testing.*` |
| `taskRunner.referenceDocs` | `referenceDocs` |
| `taskRunner.standards` | `standards` |
| `taskRunner.neverLoad` | `neverLoad` |

### 22.3 Runtime directory mapping

| Taskplane | pi-spine |
|-----------|----------|
| `.pi/batch-state.json` | `.spine/batch-state.json` |
| `.pi/runtime/{batchId}/` | `.spine/runtime/{batchId}/` |
| `.worktrees/{batchId}/` | `.worktrees/spine-{batchId}/` |
| `orch/{user}-{batchId}` | `orch/spine-{user}-{batchId}` |
...
| **5 — Dashboard + publish** | SSE dashboard, README, npm + pi.dev |
| **6 — Compatibility validation** | Gap list (`taskplane-gap-list.md`), dogfood report, regression tests for incidents I-01–I-10 |

### 23.1 Phase 0 execution policy (normative)

Do **not** repeat the 2026-05-31 failure mode:

1. Run `spine doctor` (or equivalent preflight) before any batch.
2. Commit all task packets and `dependencies.json` before `/orch` or `/spine`.
...
|------|------------|--------|------------|
| Scope creep to Tier C engine | High | High | PRD non-goals; journal at boundaries only |
| Taskplane format drift | Medium | Medium | Pinned compat tests + golden fixtures |
| pi SDK breaking changes | Medium | High | Peer dep range; doctor version check |
| Lane death / resume bugs | High | High | Heartbeats + journal reconciliation + atomic retry + `--force` |
| Stall false positive kills valid work | High | High | §18.4 progress-aware stall; FR-WORK-09/10 |
| Abort destroys recoverability | Medium | High | §18.6 archive-first abort; journal canonical |
| Dogfood with immature orchestrator | High | Medium | §23.1 serial Phase 0 policy |
...
| Version | Features |
|---------|----------|
| **v1.1** | pi-subagents backend; journal-driven batch-state rebuild; merger agent optional |
| **v1.2** | Agent-native conductor tools (`spine_run_work`); PR creation helper |
| **v2.0** | Polyrepo segments; cross-harness task posting; cloud-optional remote dashboard |

---

## 26. Appendices

### Appendix A — Review level rubric (Taskplane-compatible)

Score dimensions 0–2: blast radius, pattern novelty, security, reversibility.

| Sum | Level | Behavior |
|-----|-------|----------|
| 0–1 | 0 | No review |
| 2–3 | 1 | Plan review before code |
| 4–5 | 2 | Plan + code review |
| 6–8 | 3 | Plan + code + test review |

### Appendix B — pi-spine vs Taskplane vs pi-conductor (feature matrix)

| Feature | Taskplane | pi-conductor | pi-spine v1 |
|---------|-----------|--------------|-------------|
| PROMPT/STATUS packets | Native | Objectives | Compatible |
| Wave DAG | Yes | Yes | Yes |
| Worktree lanes | Yes | Yes | Yes |
| Event journal | Partial | Events | Yes (canonical) |
| Human gates | Implicit | Yes | Yes |
| Supervisor mail | Yes | No | No |
| Dashboard | Yes | Gate dashboard | Minimal |
| Cross-harness | No | No | No (v2) |
| Polyrepo | Yes | Partial | No (v2) |

### Appendix C — Glossary

| Term | Definition |
|------|------------|
| **Batch** | One orchestrated run of many tasks |
| **Wave** | Set of tasks runnable in parallel respecting DAG level |
| **Lane** | Git worktree + worker session executing a task chain |
...
| Document | Purpose |
|----------|---------|
| [`docs/incidents/20260531-phase0-taskplane-batch.md`](docs/incidents/20260531-phase0-taskplane-batch.md) | Phase 0 dogfood post-mortem (primary) |
| [`docs/compatibility/taskplane-gap-list.md`](docs/compatibility/taskplane-gap-list.md) | Tracked Taskplane gaps (Phase 6) |

---

## Handoff instructions

1. Create new repository `pi-spine` (or preferred org scope).
2. Copy this file to `docs/PRD.md` in that repository.
3. Implement phases in [§23](#23-implementation-phases); dogfood using PROMPT/STATUS packets per pi-spine's own compat format.
4. Do not copy consumer application code into pi-spine — application projects remain separate.
5. Read incident reports in Appendix E before designing recovery tooling.

**Product name:** pi-spine  
**npm:** `pi-spine`  
