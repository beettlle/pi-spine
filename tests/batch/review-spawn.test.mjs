...
test("spawnReviewerPi blocks nested spawn inside worker session", async () => {
	const root = await mkdtemp(path.join(os.tmpdir(), "spine-review-spawn-nested-"));
	const taskFolder = path.join(root, "spine-tasks", "SP-259-nested");
	fs.mkdirSync(taskFolder, { recursive: true });
	...
	try {
		...
		assert.equal(result.spawnFailed, true);
		assert.equal(result.reason, "nested_spawn_blocked");
	} finally {
