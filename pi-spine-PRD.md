...
pi-spine exists to reduce recurrence of this class of failure through **journal-backed reconciliation**, **explicit gates**, and **Taskplane-compatible task packets** the operator already authors.
...
| FR-BATCH-06 | `/spine-abort` graceful default; `--hard` kills lane sessions | 0 |
...
1. Load `batch-state.json` + journal tail.
2. For each task: if `.DONE` → skip; if `task.completed` in journal → skip.
3. For each lane: if heartbeat stale → respawn lane session in existing worktree.
4. Re-enter wave scheduler at `currentWaveIndex`.
5. Log `batch.resumed` with `{ resumeForced, repairedLanes }`.
...
| resume | idempotent skip, stale lane repair |
...
| Phase | Deliverables |
|-------|--------------|
| **0 — Bootstrap** | Repo, CI, pi manifest, `spine doctor`, slash stubs |
| **1 — Compat + planner** | Taskplane parsers, DAG, `/spine-plan`, fixtures |
| **2 — Single lane worker** | Worktree, AgentSession worker, STATUS, step commits |
| **3 — Multi-lane + journal** | Parallel lanes, journal, resume, status |
| **4 — Review + gates** | Review tool, integrate gate, `/spine-integrate` |
| **5 — Dashboard + publish** | SSE dashboard, README, npm + pi.dev |
| **6 — Compatibility validation** | Gap list, dogfood report |
...
| Lane death / resume bugs | High | High | Heartbeats + journal reconciliation + `--force` |
