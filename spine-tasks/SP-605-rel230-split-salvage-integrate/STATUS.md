...
1. Create `src/batch/salvage-batch-integrate.mjs` with `confirmSalvageIntegrate`, `integrateSalvageableLane`, `formatSalvageIntegrateOutput` (move from `salvage-batch.mjs`).
2. Thin `salvage-batch.mjs` to re-export list + integrate APIs (no behavior change; callers keep importing from `salvage-batch.mjs`).
